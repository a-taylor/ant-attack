...
- The captive is held in a walled yard in the **north-east** of the city. Walk in
...
| `src/city.js` | Loads `mapdata.js`, builds the instanced voxel mesh, owns the shared grid-collision physics (`moveActor`), and exposes voxel queries and key positions (spawn, captive yard, gate). |
